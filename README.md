...
- **Destructuring over *existing* bindings in for heads** (`for ([k, v] of pairs)`, `for ([a, b] in obj)` with no declaration keyword): the eagerly-parsed head expression is an array/object literal, validated as an assignment pattern by z-parser's `isValidAssignmentPattern` (same cover-grammar reinterpretation as `[a, b] = arr`) and stored as the `ForBinding.existing_pattern` variant.
...
- ~~Modules~~ — the `import`/`export` statement grammar is implemented (`ImportDecl`/`ExportDecl` StatementData variants): side-effect/default/namespace/named imports with `as` renames (keyword names like `default` allowed as imported names), `export` + declaration (delegated to parseStatement — function/class ride the existing hooks), `export { a as b } [from]`, `export default` (as an AssignmentExpression — `export default function() {}` is the anonymous function-expression form), `export * from`. Parsed permissively at any statement position; the interpreter enforces module-top-level-only. Deferred: dynamic `import()`, `import.meta`.
